...
(function($) {
	...
	$(function() {
        ...
        //사업소개 보이기/숨기기
        $("#business .business_menu_item .tit").click(function() {
            clearInterval(timer);
            
            timer = setInterval(function() {
                setBusinessSlide("auto");
            }, 8000);
            
            var oldIdx = $("#business .business_menu_item .tit.on").parent().index();
            var newIdx = $(this).parent().index();
            
            setBusinessSlide("click", oldIdx, newIdx);
        });
...
//TECHNOLOGY 라인 높이 설정
function setTechnologyHeight() {
    var technology_height = $("#technology").innerHeight();

    $("#technology .inner_line .technology_text").height(technology_height);
}

//LICENSE 라인 높이 설정
function setLicenseHeight() {
    var license_height = $("#license").innerHeight();

    $("#license .inner_line .license_text").height(license_height);
}

//BUSINESS 라인 높이 설정
function setBusinessHeight() {
    var business_height = $("#business").innerHeight();
	var business_height2 = 0;

	if ($("#business .business_menu_item").length > 0) {
		business_height2 = $("#business .business_menu_item .tit.on").next(".con").innerHeight();
	}

    $("#business .inner_line .business_text").height(business_height + business_height2);
}

//사업소개 보이기/숨기기
function setBusinessSlide(type, oldIdx, newIdx) {
    var itemCnt = $("#business .business_menu_item").length;
    
    if (itemCnt > 0) {
        if (type == "auto") {
            oldIdx = $("#business .business_menu_item .tit.on").parent().index();
            newIdx = parseInt(oldIdx) + 1;
            
            if ((oldIdx == -1) || (oldIdx == (itemCnt - 1))) {
                newIdx = 0;
            }
        }

        if ((oldIdx != newIdx) && (oldIdx > -1)) {
            $("#business .business_menu_item:eq(" + oldIdx + ") .tit").removeClass("on");
            $("#business .business_menu_item:eq(" + oldIdx + ") .con").slideUp();
        }

        $("#business .business_menu_item:eq(" + newIdx + ") .tit").addClass("on");
        $("#business .business_menu_item:eq(" + newIdx + ") .con").slideDown(function() {
			//BUSINESS 라인 높이 설정
			setBusinessHeight();
		});
    }
